...
<div class="two-col wide-left">
  <div>
    ...
    <p v-click style="margin-top: 1.1rem; font-size: 25px">По коду не видно, что этот пример плохой. Рядом лежат визард с тяжёлым расчётом прямо в рендере и страницы, которые импортируются статически. Агент просто берёт самый убедительный пример и честно угадывает. Галлюцинации тут ни при чём.</p>
  </div>
...
<!--
Время: 1:20.
Вся суть доклада: модель не знает норму проекта, она её угадывает по коду. Код — архив решений за девять лет.
...
<div class="two-col" style="margin-top: 1rem">
  <div v-click class="flat-card bad">
    ...
    <p class="muted">AI-сетап, какой бывает в жизни: AGENTS.md на 220 строк, зоопарк скиллов, «команда агентов», ревью-бот и проверка, которая ничего не проверяет.</p>
  </div>
  <div v-click class="flat-card">
...
<div class="notes">
  <div v-if="$clicks < 1" class="note intro"><b>Файл целиком — около 40 строк</b><p>Он попадает в контекст каждой задачи, поэтому здесь только то, что нужно почти всегда. Разберём по частям.</p></div>
...
<div class="notes">
  ...
  <div v-click="[4, 5]" class="note"><b>Тихая поломка</b><p>Прокси отдаёт index.html вместо файла, и ошибка всплывает совсем в другом месте. Это модель сама не выведет.</p></div>
...
<div class="practice-commit"><a href="https://github.com/spiderpoul/enterprise-app-optimization/commit/6bebdf60c409cbe60d49635f74ce96dfaaec2e7d" target="_blank">Шаг 1. AGENTS.md: карта проекта, запреты и что заденет</a></div>
...
<div class="two-col" style="align-items: start">
  <div v-click class="flat-card bad">
    <h3>Run A</h3>
    <p class="muted">Смотрим в трейсе: дочитал ли AGENTS.md на 220 строк, какой скилл взял, утащил ли хук с утечкой по совету из старой документации.</p>
  </div>
  <div v-click class="flat-card">
    <h3>Run B</h3>
...
<!--
Время: 1:40.
Главное отличие документации для агента от обычной: она явно называет плохие примеры и тихие поломки.
-->
...
<div class="practice-files">
  ...
  <div><code>docs/architecture/frontend.md</code><br>инварианты рендера, жизненного цикла и lazy-загрузки</div>
...
<!--
Время: 1:40.
В конце файла ещё раздел «Готово» с командами и парой строк, которые заранее отрезают лёгкие способы объявить «готово».
-->
...
# Места, где одна строка может положить работу всех команд.
...
critical:
  ...
  - paths: [src/shell-app/server/shell-server.js, src/shell-app/server/lib/**, 'src/microfrontends/*/server/**']
    ...
    why: сюда приходят внешние данные — регистрация без проверки превращает shell в открытый прокси
...
<div class="notes">
  ...
  <div v-click="[2, 3]" class="note"><b>Реестр, прокси, общая сборка</b><p>Отдельный ревьюер знает, что здесь ломается тихо: TTL реестра, прокси, общая сборка.</p></div>
  <div v-click="[3, 4]" class="note"><b>Безопасность</b><p>Регистрация продукта и цели прокси приходят из запроса. Реальная находка в демо: ack и удаление продукта без авторизации — любой, кто достучался до shell, подменит продукт в меню.</p></div>
...
<table class="comparison">
  ...
  <tbody>
    ...
    <tr v-click><td>Страница грузится lazy-чанком и открывается</td><td>check:bundle, страница через shell</td><td>сценарий в спеке, проверка бандла, разрешённый обход в своём микрофронте</td></tr>
...
AGENTS.md на 220 строк → правила спорят
  ↓
